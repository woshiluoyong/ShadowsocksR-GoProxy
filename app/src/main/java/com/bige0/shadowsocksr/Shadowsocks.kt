...
class Shadowsocks : AppCompatActivity()
{
	...
	override fun onCreate(savedInstanceState: Bundle?)
	{
		...
		proxyLogE.movementMethod = ScrollingMovementMethod.getInstance()
	}

	private var proxyPort: String? = null
	private fun startProxy(){
		proxyLogE.visibility = View.VISIBLE
		val err = Proxysdk.startWithLog(proxyID, "http", "") { line ->
			println("===Stephen=========proxy==log==>$line")
			if(!line.isNullOrEmpty()){
				val str = "proxy on [::]:"
				if(line.contains(str)){
					proxyPort = line.substring(line.indexOf(str)+str.length, line.indexOf("\n"))
					proxyInfoT.text = "代理信息:${Utils.getIpAddress(this)}:$proxyPort"
				}// end of if
		...
		if(err.isNullOrEmpty()){
			if(proxyPort.isNullOrEmpty())proxyInfoT.text = "代理信息:${Utils.getIpAddress(this)}"
			proxyLogE.setText("代理开启成功")
		}else{
			proxyPort = null
			proxyInfoT.text = "代理信息:未开启"
			proxyLogE.setText("代理开启失败:$err")
		}
	}

	private fun stopProxy(){
		Proxysdk.stop(proxyID)
		proxyPort = null
		proxyInfoT.text = "代理信息:未开启"
		proxyLogE.setText("代理已经关闭")
		proxyLogE.visibility = View.GONE
	}
	...
	private val proxyHandler = object : Handler() {
		override fun handleMessage(msg: Message) {
			super.handleMessage(msg)
			val line = msg.data.getString("line")
			if (++logLineCount > 100) proxyLogE.setText("")
			proxyLogE.append(line + "\n")
		}
	}
